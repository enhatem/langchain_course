...
if 'points' not in st.session_state:
    st.session_state.points = {'x': [], 'y': [], 'z': [], 'status': [], 'labels': []}

st.title("3D Surface Point Checker with Labels")
...
with st.form("point_input"):
    ...
    label = col4.text_input("Point label", value="P")
    ...
    if submitted:
        ...
        st.success(f"Point {label} ({x}, {y}, {z}) is {status} the surface (surface z: {surface_z:.2f})")
...
# Add points with color coding and labels
...
**Labels**: Text appears above points and is visible when rotating
